...
// ─── WARMUP SENDING ──────────────────────────────────────────────────────────
export async function runWarmupSending(): Promise<{ sent: number; logs: string[] }> {
  const logs: string[] = [];
  let sentCount = 0;

  try {
    // 1. Get all active warmup accounts
    const accounts = db.prepare("SELECT * FROM email_accounts WHERE status='active' AND warmup_status='active'").all() as any[];
    ...
    for (const sender of accounts) {
      ...
      if (customList.length > 0) {
        ...
        // Fallback to pool: Select another active warmup account from the database
        const otherPool = accounts.filter(a => a.id !== sender.id);
...
export async function runWarmupReceiving(): Promise<{ processed: number; logs: string[] }> {
  const logs: string[] = [];
  let processedCount = 0;

  try {
    // 1. Get all active warmup accounts to monitor their inboxes
    const accounts = db.prepare("SELECT * FROM email_accounts WHERE status='active' AND warmup_status='active'").all() as any[];
    if (accounts.length === 0) {

...
function Radio({ value, current, onChange, label, desc }) {
  ...
  return (
    <label onClick={() => onChange(value)}
      style={{ display:'flex', alignItems:'flex-start', gap:'0.75rem', padding:'0.875rem 1rem', borderRadius:10, cursor:'pointer', border:`1px solid ${active ? 'var(--accent-primary)' : 'var(--border-color)'}`, background: active ? 'rgba(99,102,241,0.07)' : 'rgba(255,255,255,0.02)', transition:'all 0.15s' }}>
  ...
  return (
    <div style={{ maxWidth:780, padding:'1.5rem 0 3rem', display:'flex', flexDirection:'column', gap:'2rem' }}>
          ...
          <div style={{ display:'flex', gap:'0.6rem', flexWrap:'wrap' }}>
            {DAYS.map(d => (
              <label key={d} style={{ display:'flex', alignItems:'center', gap:'0.45rem', cursor:'pointer', userSelect:'none',
                ...
                background: data.days[d] ? 'rgba(99,102,241,0.1)' : 'rgba(255,255,255,0.02)', transition:'all 0.15s' }}>
      ...
      {seqSteps.length === 0 && !loading && (
        <div style={{ background:'rgba(255,255,255,0.02)', border:'1px dashed var(--border-color)', borderRadius:10, padding:'1.25rem', textAlign:'center', fontSize:'0.85rem', color:'var(--text-muted)' }}>

...
const makeStep = (n) => ({
  id: Date.now() + n,
  waitDays: n === 1 ? 0 : 3,
  variations: [{ id:'A', subject:'', body:'<p>{{random|Hi|Hello|Hey}} {{first_name}},</p><p><br></p><p>- {{sender_signature}}</p>' }],
  activeVar: 0,
});
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',flex:1,minHeight:0}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:2,padding:'5px 8px',
                   ...
                   background:'rgba(255,255,255,0.02)',rowGap:4}}>
        ...
        {/* HR */}
        <button title="Divider" onMouseDown={e=>{e.preventDefault();ins('<hr style="border:none;border-top:1px solid rgba(255,255,255,0.15);margin:12px 0"/><p><br></p>');}}
...
export default function SequenceBuilder({ campaign }) {
  ...
  function addVar(si){
    setSteps(p=>p.map((s,i)=>{
      if(i!==si)return s;
      const id='ABCDEFGH'[s.variations.length]||`V${s.variations.length+1}`;
      return{...s,variations:[...s.variations,{id,subject:'',body:'<p>{{random|Hi|Hello|Hey}} {{first_name}},</p><p><br></p><p>- {{sender_signature}}</p>'}],activeVar:s.variations.length};
    }));
  }
  ...
  return(
    <div style={{display:'flex',height:'calc(100vh - 280px)',minHeight:480,position:'relative'}}>
      ...
      {/* Left: Steps */}
      <div style={{width:248,flexShrink:0,borderRight:'1px solid var(--border-color)',overflowY:'auto',padding:'1rem',background:'rgba(255,255,255,0.015)'}}>
        {loading?(
      ...
      {step&&vari&&(
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div style={{padding:'0.6rem 1rem',borderBottom:'1px solid var(--border-color)',display:'flex',alignItems:'center',gap:'0.75rem',background:'rgba(255,255,255,0.02)',flexShrink:0}}>
            ...
            <input value={vari.subject} onChange={e=>updSubj(activeStep,step.activeVar,e.target.value)}
              ...
              style={{flex:1,background:'rgba(255,255,255,0.04)',border:'1px solid var(--border-color)',borderRadius:6,color:'var(--text-primary)',padding:'5px 10px',fontSize:'0.85rem',outline:'none'}}/>
          ...
          <div style={{padding:'4px 14px',borderTop:'1px solid var(--border-color)',fontSize:'0.72rem',color:saved?'var(--success)':'var(--text-muted)',fontWeight:500}}>
            {saved?'✅ All changes saved':'● Unsaved — click Save All to persist'}
          </div>
        </div>
      )}
    </div>

    ...
    >
      ...
      <div style={{ display: 'flex', flexShrink: 0, position: 'relative' }}>
        ...
        <div style={{
          ...
          <div style={{ width: 200, display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto' }}>
            ...
            {FOLDERS.map(f => (
              <div key={f.key} onClick={() => setFolder(f.key)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.6rem 1rem', cursor: 'pointer', background: folder === f.key ? 'rgba(99,102,241,0.12)' : 'transparent', borderLeft: folder === f.key ? '3px solid var(--accent-primary)' : '3px solid transparent' }}
                onMouseEnter={e => { if (folder !== f.key) e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; }}
            ...
            {accounts.length > 0 && (<>
              ...
              {[{ email: 'all', label: '📥 All Accounts' }, ...accounts.map(a => ({ email: a.email, label: `📧 ${a.email}` }))].map(a => (
                <div key={a.email} onClick={() => setFilterAcc(a.email)} title={a.email}
                  style={{ padding: '0.45rem 1rem', cursor: 'pointer', fontSize: '0.78rem', color: filterAcc === a.email ? 'var(--accent-primary)' : 'var(--text-secondary)', background: filterAcc === a.email ? 'rgba(99,102,241,0.08)' : 'transparent', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: filterAcc === a.email ? 600 : 400 }}>{a.label}</div>
              ))}
            </>)}
      ...
      <div className={`inbox-list-panel${mobileShowDetail ? ' inbox-panel--hidden' : ''}`} style={{ width: midWidth, flexShrink: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            ...
            <>
              {accounts.length === 0 && (
                <div style={{ padding: '0.75rem 1rem', background: 'rgba(245, 158, 11, 0.1)', borderBottom: '1px solid rgba(245, 158, 11, 0.2)', color: '#f59e0b', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                return (
                  <div key={thread.key} onClick={() => { openThread(thread); setMobileShowDetail(true); }}
                    ...
                    <div style={{ display: 'flex', gap: '0.35rem', marginLeft: 40, alignItems: 'center' }}>
                      ...
                      {t.tags?.map(tag => <span key={tag} style={{ fontSize: '0.62rem', padding: '1px 6px', borderRadius: 99, background: TAG_COLORS[tag] || 'rgba(255,255,255,0.08)', color: TAG_TEXT[tag] || 'var(--text-muted)', fontWeight: 600 }}>{tag}</span>)}
                      <button onClick={ev => toggleStar(t.id, ev)} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.82rem', color: t.starred ? '#f59e0b' : 'var(--text-muted)', flexShrink: 0 }}>★</button>
                    </div>
                  </div>
                );
              })}
        ...
        {!active ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg-primary)' }}>
            ...
            <div style={{ padding: '1rem 1.5rem 0.75rem', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
              ...
              {/* Inbox badge */}
              <span style={{ fontSize: '0.68rem', padding: '2px 8px', borderRadius: 4, border: '1px solid rgba(255,255,255,0.2)', color: 'var(--text-secondary)', fontWeight: 500, flexShrink: 0 }}>
                Inbox ✕
              </span>
              {/* Nav icons */}
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem 1.5rem 1rem' }}>
              {[...threadEmails].reverse().map((msg, idx, arr) => {
                ...
                if (!isExpanded) {
                  ...
                  return (
                    <div key={msg.id} onClick={toggleExpand}
                      style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.55rem 0', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.04)' }}
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexShrink: 0 }}>
                        ...
                        <button onClick={ev => toggleStar(msg.id, ev)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.9rem', color: msg.starred ? '#f59e0b' : 'var(--text-muted)', padding: 0 }}>☆</button>
                      </div>
                    </div>
                  );
                }
                ...
                return (
                  <div key={msg.id} style={{ borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.75rem', marginBottom: '0.25rem' }}>
                    {/* Expanded header */}
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', padding: '0.75rem 0 0.5rem' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
                        <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{time}</span>
                        <button onClick={ev => toggleStar(msg.id, ev)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '1rem', color: msg.starred ? '#f59e0b' : 'var(--text-muted)', padding: 0 }}>☆</button>
                    ...
                    <div style={{ paddingLeft: 48 }}>
                      {msg.bodyLoading ? (
                        ...
                        <>
                          ...
                          {quotedText && (
                            <div>
                              <button style={{ background: 'none', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 3, padding: '1px 6px', fontSize: '0.75rem', color: 'var(--text-muted)', cursor: 'pointer', marginBottom: '0.5rem' }}>
                                · · ·
                              </button>
                            </div>
                          )}
                    ...
                    {isLast && !showReply && (
                      <div style={{ paddingLeft: 48, marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
                        <button
                          ...
                          style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'none', border: '1px solid rgba(255,255,255,0.18)', borderRadius: 6, padding: '0.4rem 1rem', fontSize: '0.82rem', color: 'var(--text-secondary)', cursor: 'pointer' }}
            ...
            {showReply && (
              <div style={{ padding: '0.75rem 1.5rem 1rem', borderTop: '1px solid var(--border-color)' }}>
                <div style={{ border: '1px solid rgba(99,102,241,0.45)', borderRadius: 10, overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,0.25)' }}>
                  <div style={{ padding: '0.5rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.06)', fontSize: '0.72rem', color: 'var(--text-muted)', display: 'flex', gap: '1.5rem' }}>
      ...
      {composeOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card" style={{ width: 600, maxWidth: '90%', background: 'var(--bg-primary)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }}>
            <div style={{ padding: '1rem 1.5rem', background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ padding: '1rem 1.5rem', background: 'rgba(255,255,255,0.02)', borderTop: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>

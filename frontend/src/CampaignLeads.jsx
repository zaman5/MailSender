    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.78)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200 }} onClick={onClose}>
      <div style={{ background:'var(--bg-secondary)', borderRadius:16, width:460, maxHeight:'75vh', display:'flex', flexDirection:'column', boxShadow:'var(--shadow-lg)' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding:'1rem 1.5rem', borderTop:'1px solid var(--border-color)', display:'flex', justifyContent:'flex-end', gap:'0.75rem', flexShrink:0 }}>
          <button className="btn btn-ghost" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" onClick={doImport} disabled={!picked}>
            {picked ? `Import ${(picked.count||0)} Leads →` : 'Select a list'}
          </button>
        </div>
      </div>
    </div>
...
export default function CampaignLeads({ campaign, active }) {
  ...
  async function handleSingleAdd(lead) {
    ...
    // Save to backend
    if (campaign?.id) {
      await api.post(`/campaigns/${campaign.id}/leads`, [colored]);
    }
                  ...
                  {actionsOpen === i && (
                    <div style={{ position: 'absolute', right: 0, top: '100%', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: 10, boxShadow: 'var(--shadow-md)', zIndex: 50, minWidth: 160, overflow: 'hidden' }}>
                      {[
                        ...
                        ['↩ Mark Replied',  () => { setLeads(prev => prev.map((x,xi) => xi===i ? {...x,status:'Replied',replied:1}:x)); setActionsOpen(null); showToast('Marked as Replied'); }],
                        ['✅ Mark Complete', () => { setLeads(prev => prev.map((x,xi) => xi===i ? {...x,status:'Completed'}:x)); setActionsOpen(null); showToast('Marked as Completed'); }],
                        ['🗑 Remove',       () => { setLeads(prev => prev.filter((_,xi) => xi!==i)); setActionsOpen(null); showToast('Lead removed'); }],
                      ].map(([label, fn]) => (

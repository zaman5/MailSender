        ...
        {step === 2 && csvData && (
          <>
            <div style={{ flex:1, overflowY:'auto', padding:'0' }}>
              {/* Column header row */}
              <div style={{ display:'grid', gridTemplateColumns:'1fr 24px 1.2fr 24px 1fr', gap:'0.5rem', padding:'0.875rem 1.5rem', borderBottom:'1px solid var(--border-color)', fontSize:'0.78rem', fontWeight:600, color:'var(--text-secondary)', background:'rgba(255,255,255,0.02)', position:'sticky', top:0 }}>
                <span>Your CSV Column</span><span></span><span>Map to Field</span><span></span><span>Sample Values</span>
              </div>

              {csvData.headers.map((h) => (
                <div key={h} style={{ display:'grid', gridTemplateColumns:'1fr 24px 1.2fr 24px 1fr', gap:'0.75rem', padding:'0.6rem 1.5rem', borderBottom:'1px solid rgba(255,255,255,0.04)', alignItems:'center' }}>
                  {/* Column name */}
            ...
            {getDupeFields().length > 0 && (
              <div style={{ margin:'0 1.5rem 0', padding:'0.6rem 1rem', background:'rgba(245,158,11,0.12)', border:'1px solid rgba(245,158,11,0.4)', borderRadius:8, fontSize:'0.78rem', color:'#f59e0b', flexShrink:0 }}>
        ...
        {step === 3 && (
          <>
            <div style={{ flex:1, overflowY:'auto', overflowX:'auto' }}>
              {/* Stats bar */}
              <div style={{ display:'flex', gap:'1rem', padding:'0.75rem 1.5rem', background:'rgba(255,255,255,0.02)', borderBottom:'1px solid var(--border-color)', fontSize:'0.78rem', flexShrink:0 }}>
                <span style={{ color:'#10b981' }}>✓ {previewRows.filter(r=>r.status==='ok').length} valid</span>
                <span style={{ color:'#f59e0b' }}>⚠ {previewRows.filter(r=>r.status==='dupe').length} duplicate</span>
                <span style={{ color:'#ef4444' }}>✕ {previewRows.filter(r=>r.status==='skip').length} invalid email</span>
                  ...
                  {previewRows.map((r) => {
                    ...
                    return (
                      <tr key={r.idx} style={{ opacity: isOk ? 1 : 0.45, background: checked && isOk ? 'rgba(99,102,241,0.04)' : 'transparent' }}
                        onMouseEnter={e => e.currentTarget.style.background = isOk ? 'rgba(255,255,255,0.03)' : 'transparent'}
                        ...
                        <td style={{ padding:'6px 8px', borderBottom:'1px solid rgba(255,255,255,0.04)', whiteSpace:'nowrap' }}>
                          {isOk
                        ...
                        {Object.entries(mapping).filter(([,v])=>v).map(([col,field])=>(
                          <td key={col} style={{ padding:'6px 10px', borderBottom:'1px solid rgba(255,255,255,0.04)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:200,

...
export default function CampaignSettings({ campaign }) {
  ...
  const [testSubject, setTestSubject]   = useState('Test Email from MailSender');
  const [testBody, setTestBody]         = useState('Hi there,\n\nThis is a test email sent from MailSender.\n\nBest regards');
  ...
  async function sendTestEmail() {
    ...
    if (!testBody.trim())           { setTestResult({ error: 'Email body is required' }); return; }
    setTestSending(true);
    setTestResult(null);
    ...
    <div style={{ maxWidth: 760, padding: '1.5rem 0 3rem 0' }}>
      ...
      <div style={{ marginTop: '2rem', borderTop: '1px solid var(--border-color)', paddingTop: '2rem' }}>
        ...
        <div className="card card-p" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          />
          {signature && (
            <div style={{ background:'rgba(99,102,241,0.06)', border:'1px solid rgba(99,102,241,0.2)', borderRadius:8, padding:'0.75rem 1rem', fontSize:'0.85rem', whiteSpace:'pre-wrap', color:'var(--text-secondary)' }}>
          ...
          {emailAccounts.length === 0 ? (
            ...
            <>
              ...
              <div className="form-group">
                ...
                <textarea className="form-input" rows={5} value={testBody} onChange={e => setTestBody(e.target.value)} style={{ resize: 'vertical' }} />
              </div>
              {testResult && (
                <div style={{

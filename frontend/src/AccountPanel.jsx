        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem' }}>
          ...
          {mainTab === 'Account' && subTab === 'Settings' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              {[['Email Address', account.email, 'text'], ['First Name', account.first_name || '', 'text'], ['Last Name', account.last_name || '', 'text'], ['Daily Sending Limit', account.limit, 'number']].map(([label, val, type]) => (
                <div key={label} className="form-group">
                  <label className="form-label">{label}</label>
                  <input className="form-input" type={type} defaultValue={val} disabled={label === 'Email Address'} />
                </div>
              ))}
              <button className="btn btn-primary" onClick={() => showToast('Basic account settings saved')}>Save Settings</button>
            </div>
          )}
